...
def test_generate_structure(cli_runner, tmp_path):
    ...
    for folder in ["connectivity", "cyclones", "dhws", "spatial", "waves"]:
        assert os.path.isdir(tmp_path / folder), f"Expected {folder} directory to be created."
    assert os.path.isfile(tmp_path / "README.md"), "README.md file was not created."
    assert os.path.isfile(tmp_path / "datapackage.json"), "datapackage.json file was not created."
...
def test_package_with_spec(mock_download_data, cli_runner, tmp_path, get_test_file):
    """
    Test the package command with a given spec file to ensure datasets are downloaded
    based on specifications.
    """
    # Run the CLI command using the spec_file fixture
    spec_file= get_test_file
    result = cli_runner.invoke(app, ["template", "package",str(tmp_path),str(spec_file)])
   
    assert result.exit_code == 0, f"Expected exit code 0, got {result.exit_code}. Output: {result.output}"
    assert mock_download_data.called, "Expected download_data to be called for datasets."
