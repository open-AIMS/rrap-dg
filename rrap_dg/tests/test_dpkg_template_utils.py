...
from rrap_dg.dpkg_template.utils import (
    create_directory_structure, load_specification, download_datasets, move_files_to_target
)
...
def test_create_directory_structure_creates_folders(tmp_path):
    ...
    assert os.path.isfile(tmp_path / "datapackage.json"), "datapackage.json was not created."
...
def test_download_datasets_successful_download(mock_download_data):
    ...
    with tempfile.TemporaryDirectory() as dest:
        ...
        mock_download_data.assert_has_calls([expected_call_1, expected_call_2], any_order=True)
